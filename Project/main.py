...
secure_llm = SecureLLM(
    bot=yandex_bot,
    LLM_INTENT_PROMPT_TEMPLATE="Определи намерение. Ввод: <<USER_INPUT>>, паттерны: <<MATCHED_PATTERNS>>",
    LLM_VALIDATION_PROMPT="Проверь корректность ответа. Input: ",
    SYSTEM_PROMPT="Ты — безопасный помощник, который отвечает строго по фактам."
)
...
async def process_message(user_message: str) -> str:
    ...
    if intent and intent.get("intent") == "malicious":
        return f"⚠️ Ввод отклонён. Причина: {intent.get('explanation', 'подозрительный ввод')}"
    ...
    if "некорректен" in validation.lower():
        return "⚠️ Ответ модели не прошёл валидацию."
